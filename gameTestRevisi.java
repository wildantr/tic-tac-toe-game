...
public class gameTestRevisi {
    ...
    private final int cols = 10; // number of cols

    private char currentPlayer; // the player x or o

    public static void main(String[] args) { //
        gameTestRevisi game = new gameTestRevisi();
        game.start();
    }
    ...
    private boolean checkRowsColumn(char a1, char a2, char a3, char a4, char a5, 
            char a6, char a7, char a8, char a9, char a10) {
        // return value by using and
        return ((a1 != '?') && (a1 == a2) && (a2 == a3) && (a3 == a4) && (a4 == a5) 
    ...
    private boolean checkRowsWinner() {
        ...
        for (int i = 0; i < rows - 1; i++) {
            if (checkRowsColumn(gameBoard[i][0], gameBoard[i][1], gameBoard[i][2], gameBoard[i][3], 
                    gameBoard[i][4], gameBoard[i][5], gameBoard[i][6], gameBoard[i][7], gameBoard[i][8], 
                    gameBoard[i][9]) == true) {
                return true;
            }
        }
    ...
    private boolean checkColsWinner() {
        ...
        for (int i = 0; i < cols; i++) {
            if (checkRowsColumn(gameBoard[0][i], gameBoard[1][i], gameBoard[2][i], gameBoard[3][i], 
                    gameBoard[4][i], gameBoard[5][i], gameBoard[6][i], gameBoard[7][i], gameBoard[8][i], 
                    gameBoard[9][i]) == true) {
                return true;
            }
        }
    ...
    private boolean checkDiagonalWinner() {
        ...
        return ((checkRowsColumn(gameBoard[0][0], gameBoard[1][1], gameBoard[2][2], gameBoard[3][3], 
                gameBoard[4][4], gameBoard[5][5], gameBoard[6][6], gameBoard[7][7], gameBoard[8][8], 
                gameBoard[9][9]) == true)
                || (checkRowsColumn(gameBoard[0][9], gameBoard[1][8], gameBoard[2][7], gameBoard[3][6], 
                        gameBoard[4][5], gameBoard[5][4], gameBoard[6][3], gameBoard[7][2], gameBoard[8][1], 
                        gameBoard[9][0]) == true));
    ...
    private void inputCharacter() {
        ...
        System.out.print("Player " + currentPlayer
                + " choose your rows (1-" + rows + "): ");
        int row = input.nextInt() - 1; // input rows and value -1 because array start with 0
        System.out.print("Player " + currentPlayer
                + " choose your cols (1-" + cols + "): ");
        int col = input.nextInt() - 1; // input cols and value -1 because array start with 0
